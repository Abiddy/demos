...
function OrbitItem({
  ...
  const captionOpacity = useTransform(() => {
    const strength = scaleStrength ? scaleStrength.get() : 0;
    const s = itemScale.get();
    if (strength < 0.55) return 0;
    const focusGate = Math.min(1, Math.max(0, (strength - 0.55) / 0.35));
    const sizeGate = Math.min(1, Math.max(0, (s - 0.82) / 0.14));
    return focusGate * sizeGate;
  });
  const captionY = useTransform(captionOpacity, [0, 1], [10, 0]);

  const offsetPath = useMotionTemplate`path("${pathValue}")`;
  const counterRotate = useTransform(rotationValue, (r) => `rotate(${-r}deg)`);
  const zIndex = useTransform(itemScale, (s) => Math.round(s * 100));
    ...
    >
      ...
      >
        ...
        {dish ? (
          ...
          >
            <div className="orbit-dish-badge">
              <p className="orbit-dish-name">{dish.name}</p>
              <p className="orbit-dish-desc">{dish.description}</p>
              <p className="orbit-dish-price">{dish.price}</p>
            </div>
          </motion.div>
        ) : null}
      </motion.div>
...
export default function OrbitImages({
  ...
  const pathValue = useTransform([currentRadiusX, currentRadiusY], ([rx, ry]) => {
    switch (shape) {
      case 'circle':
        return generateCirclePath(designCenterX, designCenterY, rx);
      case 'ellipse':
        return generateEllipsePath(designCenterX, designCenterY, rx, ry);
      case 'square':
        return generateSquarePath(designCenterX, designCenterY, rx * 2);
      case 'rectangle':
        return generateRectanglePath(designCenterX, designCenterY, rx * 2, ry * 2);
      case 'triangle':
        return generateTrianglePath(designCenterX, designCenterY, rx * 2);
      case 'star':
      ...
      case 'heart':
        return generateHeartPath(designCenterX, designCenterY, rx * 2);
      case 'infinity':
        return generateInfinityPath(designCenterX, designCenterY, rx * 2, ry * 2);
      case 'wave':
        return generateWavePath(designCenterX, designCenterY, rx * 2, ry, 3);
      case 'custom':
        return customPath || generateCirclePath(designCenterX, designCenterY, rx);
      default:
        return generateEllipsePath(designCenterX, designCenterY, rx, ry);
    }
  });
  ...
  const items = sourceList.map((src, index) => (
    <motion.img
      key={`${src}-${index}`}
      src={src}
      alt={dishes?.[index]?.name || `${altPrefix} ${index + 1}`}
      draggable={false}
    ...
    >
      ...
      >
        <motion.div
          ...
          style={{ rotate: currentRotation, x: currentTranslateX }}
        >
          {showPath && (
            <svg
              width="100%"
